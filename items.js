...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        {  
            ...
            .sort({"_id":1}).skip(itemsToSkip).limit(itemsPerPage)
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
        if (category==="All"){
            this.db.collection('item').find().count(function(err,numItems){
                 assert.equal(err,null)
                callback(numItems)    
         })     
        }
         ...
         this.db.collection('item').find({category:category}).count(function(err,numItems){
            assert.equal(err,null) 
            callback(numItems)
         })
        }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         * db.item.find( { $text: { $search: "coffee" } } )
         */
        ...
        this.db.collection('item').find( { $text: { $search: query } })
             .skip(itemsToSkip).limit(itemsPerPage)
